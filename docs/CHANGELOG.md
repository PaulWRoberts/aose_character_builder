...
|---|---|---|---|
| 2026-06-29 | Unified spells UX — one list per caster type (arcane/divine), source-labelled rows; class spells, scrolls, and spell-backed innate abilities share a single `SpellRow`; common `spell_row`/`spell_modal` macros; `spellbook_view` removed | unify-spells-ux | specs/2026-06-29-unify-spells-ux-design.md · plans/2026-06-29-unify-spells-ux.md |
| 2026-06-29 | Fix: Strict Mode no longer locks pick-only feature-choice groups (e.g. Combat Talents) — the wizard `class_setup` step rendered the group header but no selectable options under Strict (the default), leaving the draft un-completable. Strict now locks only rolled groups | unify-spells-ux | — |
| 2026-06-27 | Stackable actions + storage fixes: instance-keyed contents, world-aware containers (`_container_owner`; cross-world moves land in the owning world), single stackable-aware add/merge path (`storage.add_item`), `storage.consume_item`, one `_instance_row` feeding all contents, and a unified `stack_actions` macro (qty box → move/sell/drop/consume) for items/ammo/coins/gems | feat/stackable-actions-and-storage-fixes | 2026-06-27-stackable-actions-and-storage-fixes |
| 2026-06-27 | Inventory actions consolidation: single `/inventory/{equip,unequip,sell,charge,note,move}` dispatcher family replaces 14+ dead `/equipment/*` routes; per-instance item rows with `category`; single `inv_row_actions` macro; `magic_modal` absorbs inline modal blocks; `shop.sell_item`/`sell_from_stash` retired in favour of `sell_instance`; contract test guards template↔route contract | feat/inventory-actions-consolidation | 2026-06-27-inventory-actions-consolidation |
| 2026-06-27 | Item identity unification: flat `spec.items: list[ItemInstance]` replaces `inventory`/`stashed`/`equipped`/`loaded_ammo`/`armor_tailored`/`spec.enchanted`/`spec.ammo`; every item carries `instance_id`, `location`, `equip`, `count`, `enchantment_id`; instance-based equip/unequip/move/buy/sell; legacy string-list helpers (`buy`/`add_free`/`remove`/`remove_from_stash`) deleted | feat/item-identity-unification | 2026-06-24-item-identity-unification |
| 2026-06-24 | Equipment bug sweep: fixed doubled `equipment/` URL in magic/enchanted equip + remove forms (was 404); enchanted equip now enforces class weapon/armour allowances (weapons via `equip()`, body armour gated in-route before the flag flip); an equipped item's Move control no longer offers the PC's own carried bucket (current location is `carried`, not the literal `equipped`); UI equip-eligibility is now one source of truth — `shop.class_allows` (promoted from `_class_allows`) feeds both mundane rows and the enchanted/magic view, so the enchanted modal hides Equip → "Not usable" for disallowed gear instead of erroring on submit | main | — |
| 2026-06-24 | Spell-document bug sweep: scribe Add form is ruleset-filtered (no cantrips unless the Cantrips rule is on, no disabled-source spells); Spell Book document type hidden without the Advanced rule (+ qty stepper hidden for books); copying gated on the arcane source being deciphered (`copyable_spell_ids` ∅ until `unlocked`); `read_scroll`→`decipher_source` now deciphers captured spell books too (Read Magic); arcane scrolls show Copy once deciphered | fix/spell-document-bugs | — |
